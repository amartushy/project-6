...
class Brevets(Resource):
    def get(self):
        brevets = Brevet.objects()
        return Response(brevets.to_json(), mimetype="application/json", status=200)

    def post(self):
        body = request.get_json()
        brevet = Brevet(**body).save()
        return Response(brevet.to_json(), mimetype="application/json", status=201)
